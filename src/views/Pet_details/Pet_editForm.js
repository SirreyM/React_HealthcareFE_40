...
import { petViewConfig } from '../../utils/display_configuration';
...
const EditPetForm = () => {
  ...
  useEffect(() => {
    const fetchPetById = async () => {
      const petResponse = await makeApiCall(
        `${BASE_URL}${PATH_PET}(${id})`
      );
      const jsonResp = await petResponse.json();
  ...
  const submitForm = async () => {
    ...
    const resp = await makeApiCall(
      `${BASE_URL}${PATH_PET}(${formData.PetId})`,
      'PATCH',
            JSON.stringify({
        ...otherData,
      })
    ...
    if (resp.ok) {
      snackbar.enqueueSnackbar('Successfully updated Pet', {
        variant: 'success',
      });
      navigate({ pathname: '/pets' });
    } else {
      ...
      <Grid>
          ...
          <Box display="flex" justifyContent="space-between">
            ...
            <div className="action-buttons">
              ...
              <Button
                ...
                variant="contained"
                color="secondary"
                onClick={() => navigate({ pathname: '/pets' })}
              >
                Cancel
              </Button>
            </div>
        ...
        <Box marginTop={2} className="form-container">
          <Grid container item lg={12} xs={12}>
            {Object.keys(petViewConfig)?.map((config, ind) => (
              <>
